        ...
        <div className='md:flex flex-col h-full justify-center items-center relative w-full '>
                        ...
                        <div key={item.id}>
                            <div key={item.id} className="flex flex-row justify-between items-center mt-6 mb-6 md:mt-7 md:mb-7">
                                ...
                                <div className='flex flex-row gap-2 justify-between items-center '>
                                    ...
                                    <button onClick={() => handliClickMinus(item.id)} disabled={item.value == 0} className={` ${item.value == 0 ? "cursor-not-allowed opacity-[0.6]" : ""} text-[#333] text-2xl font-semibold border border-gray-400 w-8 h-8 rounded-full`}>
                                        -
                                    </button>

                                    {item.value}
                                    <button onClick={() => handliClick(item.id)} className='text-xl text-[#333] font-semibold border  border-gray-400 w-8 h-8 rounded-full'  >
                                        +
                                    </button>
                                </div>
                            </div>
            ...
            <div className=' mt-8 fixed bottom-0 right-0 left-0 bg-[#fff] w-full '>
                ...
                <div className='flex flex-row justify-between items-center p-3 md:p-4 '>
                    ...
                    <button onClick={() => navigate("/step2")} className='bg-gray-600 text-white px-5 py-2 md:px-8 md:py-3 rounded-lg '>
                        Next
                    </button>
                </div>
            </div>
